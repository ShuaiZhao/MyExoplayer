...
public final class DebugTextViewHelper implements Runnable {
    ...
    private static final int REFRESH_INTERVAL_MS = 1000;  //how long does it refresh the display
    ...
    public void run() {
         ...
         */
        String txt = "Shuai: Current Playing Status: ";
        txt += getRenderString();
    ...
    private String getTimeString() {
        long currentPositionms = debuggable.getCurrentPosition();
        return "Playback Position: " + currentPositionms + " ms, " + currentPositionms/1000.0 + " s";
    }

    private String getQualityString() {
        Format format = debuggable.getFormat();
        return format == null ? "Format: id:? br:? h:? "
                : "Format: id: " + format.id + " bitrate: " + format.bitrate + " height: " + format.height + " ";
    }
    ...
    private String getBandwidthString() {
        ...
        if (bandwidthMeter == null
            ...
            return "BandWidth(bps):NO_ESTIMATE ";
        } else {
            long estimatedBW = bandwidthMeter.getBitrateEstimate();
            return "Bandwidth(bps): " + estimatedBW + " bps, " +
                    estimatedBW/1000000 + " Mbps";
        }
    }

    private String getVideoCodecCountersString() {
        CodecCounters codecCounters = debuggable.getCodecCounters();
        return codecCounters == null ? "CodecCounter" : "CodecCounter: " + codecCounters.getDebugString();
    }
